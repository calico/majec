...
def sanitize_for_r(name):
    ...
    if name[0].isdigit():
        name = 'G_' + name
    # Reserved R words
    r_reserved = {'if', 'else', 'for', 'while', 'function', 'in', 'next', 'break', 
...
def parse_criteria_string(criteria_string):
    criteria = []
    if not criteria_string: return criteria
...
def fetch_count_matrix(db_conn, sample_ids_int, level='gene'):
    ...
    table_map = {
        'gene': {'tbl': 'aggregated_counts_tpm', 'feature_col': 'group_id', 'count_col': 'count'},
        'transcript': {'tbl': 'final_counts_tpm', 'feature_col': 'transcript_id_int', 'count_col': 'count'},
        'junction': {'tbl': 'junction_counts', 'feature_col': 'junction_id', 'count_col': 'read_count'} # <-- Corrected here
    }
    print(f"--- Fetching {level}-level count matrix for {len(sample_ids_int)} total samples ---")
    
    config = table_map[level]
    table, feature_col, count_col = config['tbl'], config['feature_col'], config['count_col']
    ...
    if df_long.empty:
        ...
        sys.exit(1)
    count_matrix = df_long.pivot(index=feature_col, columns='sample_id', values='count').fillna(0)
    if level in ['transcript', 'junction']:
        map_config = {'transcript': {'tbl': 'transcript_id_map', 'int': 'transcript_id_int', 'str': 'transcript_id_str'},'junction': {'tbl': 'junction_id_map', 'int': 'junction_id_int', 'str': 'junction_id_str'}}
        map_info = map_config[level]
        df_map = pd.read_sql_query(f"SELECT {map_info['int']}, {map_info['str']} FROM {map_info['tbl']}", db_conn)
        count_matrix = count_matrix.reset_index().merge(df_map, left_on=feature_col, right_on=map_info['int']).drop(columns=[feature_col, map_info['int']]).set_index(map_info['str'])
    return count_matrix.astype(int)

def fetch_confidence_metrics(db_conn, sample_ids_int, level='gene', filtered_features=None):
    ...
    if level == 'gene':
        ...
        if filtered_features:
            query += " AND gc.group_id IN ({})".format(','.join(['?'] * len(filtered_features)))
        ...
        if filtered_features:
            query += " AND t.transcript_id_str IN ({})".format(','.join(['?'] * len(filtered_features)))
            params = list(sample_ids_int) + list(filtered_features)
        else:
            params = sample_ids_int
    ...
    if level == 'gene':
        matrices = {}
        for metric in ['weighted_confidence', 'gini_coefficient', 'effective_copies', 'holistic_group_external_distinguishability', 'inter_group_competition', 'strong_evidence_fraction', 'holistic_group_distinguishability']:
            if metric in df_conf.columns:
                matrices[metric] = df_conf.pivot(index='gene_id', columns='sample_id', values=metric).apply(pd.to_numeric, errors='coerce').reindex(
        ...
        matrices = {}
        for metric in ['confidence_score', 'shared_read_fraction', 'distinguishability_score', 'discord_score', 'strong_evidence_fraction', 'completeness_penalty', 'adjusted_subset_penalty','ambiguous_fraction_distinguishability']:
            if metric in df_conf.columns:
                matrices[metric] = df_conf.pivot(index='transcript_id_str', columns='sample_id', values=metric).apply(pd.to_numeric, errors='coerce').reindex(
                index=filtered_features, 
                fill_value=np.nan
            )
        ...
        ambiguous_reliability.fillna(1.0, inplace=True) # A unique-only transcript has perfect ambiguous reliability.
        
        # 4. The final confidence is the certain part + the uncertain part scaled by its reliability.
        final_confidence_matrix = certain_confidence + (uncertain_fraction * ambiguous_reliability)
...
def fetch_and_normalize_junctions_for_psi(db_conn, sample_ids_int, gene_ids=None, scale_factor=10000, min_count_per_sample=10, min_samples=3):
    """
    ...
    if df_long.empty:
        print("ERROR: No junction data found for the selected samples/genes.", file=sys.stderr)
        return pd.DataFrame(), pd.DataFrame()

    # Create metadata before pivoting
    junction_metadata = df_long[['unique_junction_id', 'gene_id', 'junction_id_str', 
                                 'transcript_count', 'n_gene_contexts', 'all_genes']].drop_duplicates()
    ...
    if multi_gene_count > 0:
        print(f"  -> Found {multi_gene_count} junction:gene contexts from junctions shared between multiple genes")
    
    # Normalization and Pivoting
    print("  -> Normalizing counts and pivoting to matrix format...")
    
    df_long['normalized_count'] = \
        ((df_long['read_count'] + 1) / (df_long['total_gene_junction_reads'] + 10) * scale_factor)
    
    psi_counts_matrix = df_long.pivot(
        index='unique_junction_id', 
        columns='sample_id', 
        values='normalized_count'
    ).fillna(0).round().astype(int)
    
    print(f"  -> Final matrix: {psi_counts_matrix.shape[0]} junction:gene contexts x {psi_counts_matrix.shape[1]} samples")
...
def filter_low_expression(counts_df, min_cpm=1.0, min_samples=3):
    ...
    print(f"    (Kept features with >= {min_cpm} CPM in at least {min_samples} samples)")
    print(f"    Removed {counts_df.shape[0] - filtered_counts_df.shape[0]:,} low-expression features")
    print(f"    Median library size: {counts_df.sum(axis=0).median():,.0f}")
    print(f"    Library size range: {counts_df.sum(axis=0).min():,.0f} - {counts_df.sum(axis=0).max():,.0f}")

    
    return filtered_counts_df


def create_variance_matrix_from_confidence(counts_matrix, confidence_matrix, level='gene', variance_base=1):
    # Align matrices
    ...
    if level=='gene':
        ...
        variance_inflation = variance_base ** (1 - conf_aligned) # max 10--fold variance boost.
...
def generate_deseq2_script_with_options(comparisons, output_prefix, confidence_mode='none', level='gene', batch_column=None):
    """
...
def build_comparison_columns(all_metadata, comparisons, group_to_sample_map):
    ...
    for comp_name, case_group, control_group in comparisons:
        ...
        if case_group in group_to_sample_map:
            ...
            all_metadata.loc[all_metadata['sample_id_int'].isin(case_samples), col_name] = 'case'
        
        # Mark control samples  
        if control_group in group_to_sample_map:
            control_samples = group_to_sample_map[control_group]
            all_metadata.loc[all_metadata['sample_id_int'].isin(control_samples), col_name] = 'control'
...
def main():
    ...
    cohort_group.add_argument("--cohort_str", help="Define a baseline cohort of samples to include for statistical modeling.")
    cohort_group.add_argument("--cohort_file", help="Define a baseline cohort from a file.")
    parser.add_argument("--comparisons_file", required=True, help="Path to a 3-column file defining contrasts (comparison_name;case_group;control_group).")
    parser.add_argument("--level", choices=['gene', 'transcript', 'junction', 'delta_psi'], default='gene', 
        help="Feature level for DE analysis. 'delta_psi' enables junction normalization.")
    parser.add_argument("--output_dir", required=True, help="Directory to write all output files.")
    parser.add_argument("--output_prefix", help="Optional prefix for all output files. (Default: the analysis level)")
    parser.add_argument("--confidence_mode", choices=['none', 'append', 'variance'], 
                   default='append',
                   help="How to use MAJEC confidence: none, append metrics only, or use for variance. Only functions at gene or transcript level")
    parser.add_argument("--variance_base", help="A constant to determine how big an effect confidence has on the adjusted variance used in DESeq2\n"
                        "Higher values increase implied variance for low confidence features. The param value is the maximum variance multiplier. Deafult = 5", default=5)
    parser.add_argument("--min_cpm", type=float, default=1.0, help="Minimum CPM for pre-filtering.")
    parser.add_argument("--min_samples", type=int, default=3, help="Minimum number of samples for CPM pre-filtering.")
    parser.add_argument("--junction_min_average_count", type=float, default=10, help="Minimum average junction count for a junctions inclusion in delta psi.")
    parser.add_argument("--junction_min_samples", type=int, default=3, help="Minimum number of samples with the junction observed for its inclusion in delta psi.")
    parser.add_argument("--min_gene_cpm", type=int, default=2, help="Minimum CPM for gene-based pre-filtering of junctions.")
    parser.add_argument("--min_gene_samples", type=int, default=3, help="Minimum number of samples above CPM threshold for gene-based pre-filtering of junctions.")
    parser.add_argument("--batch_column", help="Column name for batch effects (will add to DESeq2 design)")

    args = parser.parse_args()
    if args.level in ['junction', 'delta_psi'] and args.confidence_mode != 'none':
        print(f"WARNING: Confidence metrics are not available for {args.level} level analysis.")
    ...
    if args.cohort_str or args.cohort_file:
        ...
        cohort_criteria = parse_criteria_string(args.cohort_str) or parse_criteria_file(args.cohort_file)
    ...
    for name, case_group, control_group in comparisons:
        if case_group not in group_definitions:
            print(f"ERROR: Case group '{name}' not defined in groups file", file=sys.stderr)
            sys.exit(1)
        if control_group not in group_definitions:
            print(f"ERROR: Control group '{name}' not defined in groups file", file=sys.stderr)
            sys.exit(1)
    ...
    all_metadata = pd.read_sql_query(
        ...
        con, params=final_sample_ids
    )
    
    # Build comparison-specific columns
    all_metadata = build_comparison_columns(all_metadata, comparisons, group_to_sample_map)
    ...
    if args.level == 'delta_psi':
        ...
        """
        df_gene_counts_long = pd.read_sql_query(gene_counts_query, con, params=final_sample_ids)
        # Pivot to a wide matrix for filtering
        gene_counts_matrix = df_gene_counts_long.pivot(index='group_id', columns='sample_id', values='count').fillna(0)
        ...
        counts_matrix, junction_metadata = fetch_and_normalize_junctions_for_psi(con, final_sample_ids, gene_ids=genes_to_analyze, 
                min_count_per_sample=args.junction_min_average_count, min_samples=args.junction_min_samples)
        # Save junction metadata
        junction_metadata_path = os.path.join(args.output_dir, f"{output_prefix}_junction_metadata.tsv")
    ...
    if args.level == 'delta_psi': # Skip CPM filtering for normalized junction data
        ...
        filtered_counts = filter_low_expression(counts_matrix, args.min_cpm, args.min_samples)
    ...
    if args.confidence_mode != 'none' and args.level in ['gene', 'transcript']:
        ...
        for metric_name, metric_matrix in all_metrics.items():
            metric_path = os.path.join(args.output_dir, f"{output_prefix}_{metric_name}_matrix.tsv")
            metric_matrix.to_csv(metric_path, sep='\t')

        
        if args.confidence_mode == 'variance':
            # Also save variance matrix
            variance_matrix = create_variance_matrix_from_confidence(filtered_counts, confidence_matrix, level=args.level, variance_base=args.variance_base)
            var_path = os.path.join(args.output_dir, f"{output_prefix}_variance_matrix.tsv")
            variance_matrix.to_csv(var_path, sep='\t')

    r_script = generate_deseq2_script_with_options(comparisons, output_prefix, confidence_mode=args.confidence_mode, level=args.level, batch_column=args.batch_column)
    ...
    with open(r_script_path, 'w') as f: f.write(r_script)
