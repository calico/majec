...
try:
    ...
    print("Warning: Install kaleido for SVG/PDF export capability. pip install kaleido or conda install python-kaleido")
...
def fetch_gene_data_raw(db_conn, gene_id_or_transcript_id, sample_ids):
    ...
    """
    print(f"Fetching raw data for '{gene_id_or_transcript_id}' across {len(sample_ids)} samples...")
    
    # --- Step 1: Resolve ID to gene_id ---
    df_check = pd.read_sql_query(
        "SELECT gene_id FROM transcript_annotations WHERE transcript_id_str = ? OR gene_id = ?", 
        db_conn, params=(gene_id_or_transcript_id, gene_id_or_transcript_id)
    )
    if not df_check.empty:
        gene_id = df_check['gene_id'].iloc[0]
        print(f"  -> Resolved to gene: '{gene_id}'")
    else:
        print(f"  ERROR: Identifier '{gene_id_or_transcript_id}' not found.", file=sys.stderr)
        return None
...
def fetch_gene_data_raw_with_junction_context(db_conn, gene_id_or_transcript_id, sample_ids):
    """
    ...
    unique_junction_ids = raw_data['junctions_long']['junction_id_int'].unique().tolist()
    ...
    raw_data['junctions_long']['all_genes'] = raw_data['junctions_long']['junction_id_str'].map(
        lambda x: '; '.join(junction_to_genes.get(x, []))
    )
    raw_data['junctions_long']['n_genes'] = raw_data['junctions_long']['junction_id_str'].map(
        lambda x: junction_to_n_genes.get(x, 1)
    )
    
    if len(multi_gene_junctions) > 0:
        print(f"  -> Found {len(multi_gene_junctions)} junctions shared with other genes: {', '.join(other_genes)}")
...
def aggregate_replicate_data(raw_data, group_A_ids, group_B_ids=None):
    ...
    def assign_group(sample_id):
        if sample_id in group_A_ids: return 'A'
        if group_B_ids and sample_id in group_B_ids: return 'B'
        return None 
    ...
    agg_functions={'transcripts' : {
        ...
        "aggregated":{'norm_count':['mean', 'std', 'count'], 'tpm':['mean', 'std', 'count']}
                  }
    agged_dfs={}
    for key in data_to_normalize:
        ...
        if key == "aggregated":
            ...
            df_agg = df.groupby(['transcript_id_int','junction_id_str','group']).agg(agg_functions[key])
            ...
            df_agg = df.groupby(['transcript_id_int','territory_role','start','end','group']).agg(agg_functions[key])
            df_unstacked = df_agg.unstack()
            df_unstacked.columns = ['_'.join(col).strip() for col in df_unstacked.columns.values]
            agged_dfs[f"{key}_agg"]=df_unstacked
            ...
            df_unstacked.columns = ['_'.join(col).strip() for col in df_unstacked.columns.values]
            agged_dfs[f"{key}_agg"]=pd.merge(raw_data['annotations'], df_unstacked, on="transcript_id_int", how="left")
        
    # Add junction-gene map info to aggregated df
    if 'junctions_agg' in agged_dfs and 'all_genes' in raw_data['junctions_long'].columns:
        # Get unique junction metadata from long form
        junction_metadata = raw_data['junctions_long'][['junction_id_str', 'all_genes', 'n_genes']].drop_duplicates()
        ...
        junction_index_level = agged_dfs['junctions_agg'].index.get_level_values('junction_id_str')
        ...
        agged_dfs['junctions_agg']['all_genes'] = agged_dfs['junctions_agg']['all_genes'].fillna('')
        agged_dfs['junctions_agg']['n_genes'] = agged_dfs['junctions_agg']['n_genes'].fillna(1)
...
def calculate_junction_confidence_intervals(agged_dfs, raw_data, n_boot=1000):
    ...
    df_junctions_long = df_junctions_long.merge(df_totals, on='sample_id_int', how='left')
    df_junctions_long['psi'] = df_junctions_long['norm_read_count'] / (df_junctions_long['total_reads'] + 1e-6)
    ...
    for (tid, junc_id) in agged_dfs['junctions_agg'].index:
        ...
        if len(psi_A) >= 2 and len(psi_B) >= 2:
            ...
            mean_delta = np.log2((np.mean(psi_B) + 0.001) / (np.mean(psi_A) + 0.001)) if len(psi_B) > 0 else 0
            ci_lower = np.nan
            ci_upper = np.nan
        ...
        ci_results.append({
            ...
            'is_significant': not np.isnan(ci_lower) and (ci_lower > 0 or ci_upper < 0)
        })
...
def calculate_junction_statistics(agged_dfs, raw_data):
    ...
    junctions_by_group = df_junctions_long.groupby(['transcript_id_int', 'junction_id_str', 'group'])
    ...
    for (tid, junc_id) in df_junctions_long[['transcript_id_int', 'junction_id_str']].drop_duplicates().values:
        counts_A = df_junctions_long[(df_junctions_long['transcript_id_int']==tid) & 
                                     (df_junctions_long['junction_id_str']==junc_id) & 
                                     (df_junctions_long['group']=='A')]['norm_read_count'].values
        counts_B = df_junctions_long[(df_junctions_long['transcript_id_int']==tid) & 
                                     (df_junctions_long['junction_id_str']==junc_id) & 
                                     (df_junctions_long['group']=='B')]['norm_read_count'].values
        ...
        if len(counts_A) >= 3 and len(counts_B) >= 3:
            ...
            total_A = df_junctions_long[(df_junctions_long['group']=='A')].groupby('sample_id_int')['norm_read_count'].sum()
            total_B = df_junctions_long[(df_junctions_long['group']=='B')].groupby('sample_id_int')['norm_read_count'].sum()
        ...
        # Option 2: Mann-Whitney U for small sample sizes
            ...
            u_stat, p_value = stats.mannwhitneyu(counts_A, counts_B, alternative='two-sided')
            t_stat = u_stat
            cohens_d = np.nan
        ...
        # Option 3: Fisher's exact test for presence/absence if very few replicates
            ...
            odds_ratio, p_value = stats.fisher_exact([[present_A, absent_A], 
                                                      [present_B, absent_B]])
            t_stat = odds_ratio
            cohens_d = np.nan
    ...
    if len(df_stats) > 0:
        _, df_stats['p_adjusted'], _, _ = multipletests(df_stats['p_value'], 
                                                         method='fdr_bh', 
                                                         alpha=0.05)
...
def calculate_expression_statistics(agged_dfs, norm_data):
    ...
    if 'aggregated_agg' in agged_dfs and not df_aggregated_long.empty:
        ...
        gene_counts_A = df_aggregated_long[df_aggregated_long['group'] == 'A']['norm_count'].dropna().values
        gene_counts_B = df_aggregated_long[df_aggregated_long['group'] == 'B']['norm_count'].dropna().values
        ...
        if len(gene_counts_A) > 0 and len(gene_counts_B) > 0:
            ...
            if len(gene_counts_A) >= 2 and len(gene_counts_B) >= 2:
                _, gene_p_value = stats.ttest_ind(gene_counts_A, gene_counts_B, equal_var=False)
            else:
                gene_p_value = 1.0
        else:
            gene_expr_lfc = 0
...
def create_junction_arc_plot_aggregated(agged_dfs, raw_data, gene_id, track_spacing=3):
    ...
    sort_col = 'tpm_mean_A' if 'tpm_mean_A' in df_transcripts.columns else 'norm_count_mean_A'
    df_transcripts_sorted = df_transcripts.sort_values(by=sort_col, ascending=True, na_position='first')
    ...
    max_tpm = pd.to_numeric(df_transcripts['tpm_mean_A'], errors='coerce').max() if 'tpm_mean_A' in df_transcripts.columns else 1.0
    max_read_count = df_junctions['norm_read_count_mean_A'].max() if 'norm_read_count_mean_A' in df_junctions.columns else 1.0
    if has_sample_B:
        max_read_count = max(max_read_count, df_junctions['norm_read_count_mean_B'].max())
    ...
    for _, tx_row in df_transcripts_sorted.iterrows():
        ...
        for _, junc_row in tx_junctions.iterrows():
            ...
            base_arc_height = 0.2
            arc_height_A = base_arc_height + 1.5 * (np.log1p(read_mean_A) / np.log1p(max_read_count))
            ...
            if has_sample_B:
                ...
                if 'delta_psi_lfc' in junc_row:
                    hover_text += f"<br><b>ΔPSI (log2):</b> {junc_row['delta_psi_lfc']:.2f}"
                    if 'delta_psi_ci_lower' in junc_row and not pd.isna(junc_row['delta_psi_ci_lower']):
                ...
                #     arc_height_above = base_arc_height + 1.5 * (np.log1p(read_mean_A) / np.log1p(max_read_count))
                #     arc_height_below = base_arc_height + 1.5 * (np.log1p(read_mean_B) / np.log1p(max_read_count))
                    
                #     all_shapes.append({
                #         'type': 'rect',
                #         'x0': x0 - 0.1,
                #         'x1': x1 + 0.1,
                #         'y0': y_position - arc_height_below - 0.2,  
                #         'y1': y_position + arc_height_above + 0.2,  
                ...
                # Sample B arc
                if has_sample_B:
                    arc_height_B = base_arc_height + 1.5 * (np.log1p(read_mean_B) / np.log1p(max_read_count))
                    ...
                    if 'delta_psi_lfc' in junc_row and abs(junc_row['delta_psi_lfc']) > 0.1:
                        bar_x = x0 + 0.5 * (x1 - x0)
                        bar_height = abs(junc_row['delta_psi_lfc']) * 0.3
                        bar_color = '#FF6B35' if junc_row['delta_psi_lfc'] > 0 else '#00A6ED'
                        ci_fill_color = f"rgba(255, 107, 53, 0.2)" if junc_row['delta_psi_lfc'] > 0 else f"rgba(0, 166, 237, 0.2)"
                        ...
                        if 'delta_psi_ci_lower' in junc_row and not pd.isna(junc_row['delta_psi_ci_lower']):
                            ...
                            all_shapes.append({
                                'type': 'line',
                                'x0': bar_x - 0.05, 'x1': bar_x + 0.05,
                                'y0': y0_ci_line, 'y1': y0_ci_line,
                                'line': {'color': 'black', 'width': 1}
                            })
...
def create_differential_junction_heatmap(agged_dfs, gene_id):
    ...
    for _, transcript_row in df_transcripts.iterrows():
        ...
        transcript_junctions = df_junctions[df_junctions['transcript_id_int'] == tid_int]
        ...
        for junc in sorted_junctions:
            junc_data = transcript_junctions[transcript_junctions['junction_id_str'] == junc]
            
            if not junc_data.empty:
                if 'delta_psi_lfc' in junc_data.columns:
                    delta_psi = junc_data['delta_psi_lfc'].iloc[0]
                    is_sig = junc_data['is_significant'].iloc[0] if 'is_significant' in junc_data.columns else False
                    # Get the mean counts
                    mean_count_A = junc_data['norm_read_count_mean_A'].iloc[0] if 'norm_read_count_mean_A' in junc_data.columns else 0
                    mean_count_B = junc_data['norm_read_count_mean_B'].iloc[0] if 'norm_read_count_mean_B' in junc_data.columns else 0
        ...
        transcript_info.append({
            ...
            'expr_lfc': transcript_row.get('expr_lfc', 0),  # Make sure this matches the column name
    ...
    sorted_indices = sorted(range(len(transcript_info)), 
                          key=lambda i: -transcript_info[i]['mean_tpm'])
    ...
    fig = make_subplots(
        ...
        subplot_titles=['TPM A', 'TPM B','LFC', f' Differential Junction Usage: {gene_id}'],
    ...
    for i in range(heatmap_array.shape[0]):
        ...
        for j in range(heatmap_array.shape[1]):
            ...
            if n_jgenes_array[i,j] > 1:
                hover_text += f"<br><b>⚠ Multi-gene junction ({n_jgenes_array[i,j]} genes)</b>"
    ...
    fig.update_layout(annotations=fig.layout.annotations + tuple(annotations) if fig.layout.annotations else annotations)
    # Update y-axis with transcript names
    y_labels = [t['tid_str'] for t in transcript_info_sorted[:-1]] + ['<b>Gene Total</b>']
    for col in [1, 2, 3]:
        fig.update_yaxes(
...
def create_junction_penalty_heatmap_plotly(gene_data, sample_name='A', output_prefix=None):
    """
    ...
    for _, transcript_row in df_transcripts.sort_values('tpm').iterrows():
        ...
        for junc in sorted_junctions:
            if junc in expected_junctions:
                ...
                row_hover.append(f"Junction: {junc}<br>Count: {count}<br>Transcript: {tid_str}")
            else:
                row_values.append(np.nan)
        ...
        transcript_info.append({
            'tid_int': tid_int,
            'tid_str': tid_str,
            'tsl': int(tsl) if (tsl := transcript_row.get('tsl')) and str(tsl).isdigit() else np.nan,
            ...
            'adjusted_subset_penalty': transcript_row.get('adjusted_subset_penalty', 1.0),
    ...
    sorted_indices = sorted(range(len(transcript_info)), 
                          key=lambda i: (transcript_info[i]['tpm']))
    ...
    log_heatmap = np.where(~np.isnan(heatmap_array), 
                           np.log2(heatmap_array + 1), 
                           np.nan)
    ...
    fig = make_subplots(
        ...
        subplot_titles=['TSL', 'Sub', 'Comp', 'Init', 'Final', f'Junction Coverage: {gene_id}'],
        horizontal_spacing=0.02
    )
    ...
    subset_penalties = [[info['adjusted_subset_penalty']] for info in transcript_info_sorted]
    subset_text = [['Y' if info['is_subset'] else 'N'] for info in transcript_info_sorted]
    ...
    completeness_penalties = [[info['completeness_penalty']] for info in transcript_info_sorted]
    completeness_text = [[f"{info['n_observed']}/{info['n_expected']}"] 
                        for info in transcript_info_sorted]
    ...
    if len(sorted_junctions) <= 30:
        ...
        x_ticktext = [f'J{i}' if i % step == 0 else '' 
                     for i in range(len(sorted_junctions))]
...
def draw_transcript_bilateral(fig, all_junctions, transcript_id, transcript_info, 
                             subset_tid, row, junction_color_a='rgba(150, 50, 50, 0.8)', 
    ...
    max_arc_height = 0.4
    max_read_count_a = all_junctions['norm_read_count_mean_A'].max() if 'norm_read_count_mean_A' in all_junctions.columns else 1
    max_read_count_b = all_junctions['norm_read_count_mean_B'].max() if 'norm_read_count_mean_B' in all_junctions.columns else 1
    ...
    for idx, junc in tx_junctions.iterrows():
        ...
        if not pd.isna(mean_a):
            ...
            if mean_a > 0:
                ...
                y_points = arc_height * np.sin(np.pi * (x_points - start) / (end - start))
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=y_points,
                        mode='lines',
                        line=dict(color=junction_color_a, width=max(1, min(3, mean_a/10))),
            ...
            if mean_b > 0:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=y_points,
                        mode='lines',
                        line=dict(color=junction_color_b, width=max(1, min(3, mean_b/10))),
                        hovertext=junction_hover,
                        hoverinfo='text',
                        showlegend=False
                    ),
...
def visualize_subset_focused_territories(
    ...
    # Get subset info
    subset_info = df_transcripts[df_transcripts['transcript_id_int'] == subset_tid].iloc[0]
    ...
    if pd.isna(penalty_a) or pd.isna(penalty_b) or (penalty_a > 0.99 and penalty_b > 0.99):
    ...
    all_junctions = df_junctions[
        df_junctions.index.get_level_values('transcript_id_int').isin(all_transcript_ids)
    ].copy()
    ...
    if not all_junctions.empty:
        for junc_str in all_junctions.index.get_level_values('junction_id_str').unique():
            start, end = parse_junction_coords(junc_str)
    ...
    if not subset_territories.empty:
        all_coords.extend(subset_territories.index.get_level_values('start').tolist() + 
                         subset_territories.index.get_level_values('end').tolist())
    if not comparator_territories.empty:
        all_coords.extend(comparator_territories.index.get_level_values('start').tolist() + 
    ...
    transcript_height = min(0.08, 0.4 / n_transcripts)  # Cap individual transcript height
    ...
    for i in range(n_transcripts):
        ...
        if i == 0:
            ...
            row_specs.append([{"colspan": 2}, None, {"rowspan": n_transcripts + 1, "secondary_y": True}])
    ...
    row_specs.append([{"colspan": 2}, None, None])  # Third column occupied by metrics panel
    subplot_titles.extend(['', None, None])
    ...
    draw_transcript_bilateral(fig, all_junctions, subset_tid, subset_info, subset_tid, 1)
    
    # Draw each superset
    for i, (_, superset_info) in enumerate(superset_infos.iterrows(), start=2):
        draw_transcript_bilateral(fig, all_junctions, superset_info['transcript_id_int'], superset_info, subset_tid, i)
    ...
    if not subset_territories.empty:
        for idx, terr in subset_territories.iterrows():
            ...
            if not pd.isna(cov_b):
                ...
                fig.add_trace(
                    go.Scatter(
                        x=[start + offset, end - offset, end - offset, start + offset, start + offset],
                        y=[0, 0, log_cov, log_cov, 0],
                        fill='toself',
    ...
    for i, (metric, label, use_log, secondary_y) in enumerate(metrics):
        ...
        if not raw_transcripts.empty and f'{metric}' in raw_transcripts.columns:
            for condition, color, x_offset in [('A', junction_color_a, -0.2), ('B', junction_color_b, 0.2)]:
                ...
                if not condition_data.empty and metric in condition_data.columns:
                    ...
                    if len(sample_values) > 0:
                        display_values = -np.log10(sample_values + 1e-10) if use_log else sample_values
                        ...
                        fig.add_trace(
                            go.Violin(
                                ...
                                jitter=0.05,
                                orientation='v',
                                hovertext=[f"{label} {condition}<br>Sample: {s}<br>Value: {v:.4f}" + 
                                          (f"<br>-log10: {-np.log10(v + 1e-10):.2f}" if use_log else "")
                                          for s, v in zip(condition_data['sample_id_x'], sample_values)],
                                hoverinfo='text',
                                showlegend=False,
                                scalegroup=f'{metric}',
                                scalemode='width'
                            ),
                            row=1, col=3, secondary_y=secondary_y
                        )
                        plot_created = True
        ...
        if not plot_created:
            for condition, color, x_offset in [('A', junction_color_a, -0.2), ('B', junction_color_b, 0.2)]:
                ...
                if not pd.isna(val_mean):
                    ...
                    y_err = abs(-np.log10(max(val_mean - val_std, 1e-10)) - y_val) if (use_log and val_std > 0) else val_std
                    ...
                    fig.add_trace(
                        go.Bar(
                            ...
                            y=[y_val],
                            width=0.3,
                            error_y=dict(type='data', array=[y_err]) if val_std > 0 else None,
                            marker_color=color,
    ...
    fig.add_annotation(
        ...
        xref='paper', yref='paper',
        x=0.35,
        y=1 - (cumulative_heights[n_transcripts] / total_height_units) - .05,  # Above territory row
        showarrow=False,
        font=dict(size=11),
        xanchor='center'
    )
    ...
    penalty_max = max(-np.log10(subset_info.get('adjusted_subset_penalty_mean_A', 0.1) + 1e-10),
    ...
    fig.update_yaxes(
        range=[0, penalty_max * 2],  # Double the range
        row=1, col=3, 
        secondary_y=False
    )
    
    fig.update_yaxes(
        range=[0, tpm_max * 2],  # Double the range
        row=1, col=3, 
        secondary_y=True
    )
    
    fig.update_yaxes(
        title_text="Penalty / Evidence (-log10)", 
        row=1, col=3, 
        secondary_y=False 
    )
    fig.update_yaxes(
    ...
    fig.add_trace(go.Scatter(x=[None], y=[None], mode='markers', 
                            ...
                            name='Condition A', showlegend=True))
    fig.add_trace(go.Scatter(x=[None], y=[None], mode='markers',
                            marker=dict(size=10, color=junction_color_b),
                            name='Condition B', showlegend=True))
    fig.add_trace(go.Scatter(x=[None], y=[None], mode='markers',
                            marker=dict(size=10, color=territory_unique_color_a),
                            name='Unique Territory A', showlegend=True))
    fig.add_trace(go.Scatter(x=[None], y=[None], mode='markers',
                            marker=dict(size=10, color=territory_comp_color_a),
                            name='Comparator Territory A', showlegend=True))
...
def visualize_subset_junctions_only(
    ...
    subset_junctions_set = set(all_junctions[
    ...
    fig = make_subplots(
        ...
        vertical_spacing=0.01,
        horizontal_spacing=0.07,
        subplot_titles=[''] * ((n_transcripts + 1) * 3)  # Empty titles, we'll add as annotations
    )
    
    # Color schemes
    junction_color_a = 'rgba(150, 50, 50, 0.8)'
    junction_color_b = 'rgba(50, 50, 150, 0.8)'
    subset_junction_color = 'rgba(50, 150, 50, 0.1)'  # Green background for subset junctions
    ...
    for junc_id in sorted_junction_ids:
        junc_data = all_junctions[all_junctions.index.get_level_values('junction_id_str') == junc_id]
        if not junc_data.empty:
            mean_a = junc_data['norm_read_count_mean_A'].mean() if 'norm_read_count_mean_A' in junc_data.columns else 0
            mean_b = junc_data['norm_read_count_mean_B'].mean() if 'norm_read_count_mean_B' in junc_data.columns else 0
            std_a = junc_data['norm_read_count_std_A'].mean() if 'norm_read_count_std_A' in junc_data.columns else 0
            std_b = junc_data['norm_read_count_std_B'].mean() if 'norm_read_count_std_B' in junc_data.columns else 0
    ...
    for i, junc_id in enumerate(sorted_junction_ids):
        ...
        bg_color = subset_junction_color if category == 'subset' else superset_only_color
    ...
    for i, junc_id in enumerate(sorted_junction_ids):
        ...
        if not junc_data.empty:
            ...
            if not raw_junctions.empty and show_individual_samples:
                ...
                for condition, color, x_offset in [('A', junction_color_a, -0.15), ('B', junction_color_b, 0.15)]:
                    condition_data = junc_raw[junc_raw['group'] == condition] if 'group' in junc_raw.columns else pd.DataFrame()
                    
                    # Using size factor normalzied junction read counts
                    if not condition_data.empty and 'norm_read_count' in condition_data.columns:
                        values = condition_data['norm_read_count'].dropna()
                        if len(values) > 0:
                            # Hover for violin plots
                            category_text = "Subset junction" if junction_categories[junc_id] == 'subset' else "Superset-only junction"
                            hover_texts = []
                            for idx, val in enumerate(values.values):
                                sample_id = condition_data.iloc[idx]['sample_id_x'] if 'sample_id_x' in condition_data.columns else f"Sample {idx+1}"
            ...
            for condition, color, x_offset in [('A', junction_color_a, -0.2), ('B', junction_color_b, 0.2)]:
                ...
                if not condition_data.empty and metric in condition_data.columns:
                    ...
                    if len(sample_values) > 0:
                        ...
                        hover_texts = []
                        for idx, (val, disp_val) in enumerate(zip(sample_values.values, display_values)):
                            sample_id = condition_data.iloc[idx]['sample_id_x'] if 'sample_id_x' in condition_data.columns else f"Sample {idx+1}"
    ...
    fig.update_xaxes(
        ...
        ticktext=[j.split(':')[1] if len(j.split(':')[1]) < 20 else '...' for j in sorted_junction_ids],
        tickangle=45,
...
def plot_all_subsets_junction_focus(aggregated_data: Dict, raw_data: Dict, 
                                   show_individual_samples: bool = True,
    ...
    for subset_tid in subset_ids:
        ...
        if has_territory and not include_territory_subsets:
            print(f"Skipping {subset_name} - has territory data (use territory visualization instead)")
            continue
...
def plot_all_subsets_with_territories(aggregated_data: Dict, raw_data: Dict, show_individual_samples: bool = True):
    """
    ...
    for subset_tid in subset_ids:
        ...
        print(f"Creating territory visualization for subset {subset_name} with all its supersets")
...
def export_multipanel_with_navigation(figure_list, sample_names, output_file="output.html"):
    """
    ...
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        ...
        for key, df in aggregated_data.items():
            if isinstance(df, pd.DataFrame):
                ...
                if isinstance(df.columns, pd.MultiIndex):
                    ...
                    df_copy = df.copy()
                    df_copy.columns = ['_'.join(map(str, col)).strip() for col in df.columns.values]
        ...
        for key, df in {**aggregated_data, **norm_data}.items():
            if isinstance(df, pd.DataFrame):
                ...
                readme_data['Description'].append(get_description(key))  # Helper function
...
def make_summary_data(aggregated_data, norm_data):
    summary = {
        ...
        'Value': [
            ...
            sum(aggregated_data.get('transcripts_agg', pd.DataFrame())['adjusted_subset_penalty_mean_A'] < 0.99),
...
def process_related_genes_subprocess(raw_data, current_args, output_dir):
    ...
    with open(os.path.join(output_dir, f"{raw_data['gene_id']}_related_genes_summary.json"), 'w') as f:
    ...
    for arg in ['db', 'group_A_str', 'group_A_file', 'group_B_str', 'group_B_file', 'sample_list', 'delta_psi_bootstrap']:
    ...
    for i, gene_id in enumerate(related_genes, 1):
        print(f"  [{i}/{len(related_genes)}] Processing {gene_id}...", end='', flush=True)
        
        gene_cmd = base_cmd + base_args + [
            '--gene', gene_id,
...
def main():
    parser = argparse.ArgumentParser(description="Generate interactive analysis plots from a MAJEC database.")
    # --- Inputs ---
    parser.add_argument("--db", required=True, help="Path to the MAJEC SQLite database.")
    parser.add_argument("--gene", required=True, help="Gene ID or transcript ID to analyze.")
    parser.add_argument("--output_dir", required=True, help="Directory to save the output HTML files.")
    parser.add_argument("--delta_psi_bootstrap", type=int, default=1000, help="Number of bootstrap iterations for delta PSI confidence intervals.")
    parser.add_argument("--save_svg", action='store_true', help="Also save in SVG format.")
    parser.add_argument("--save_pdf", action='store_true', help="Also save in pdf format.")
    parser.add_argument('--export_excel', action='store_true', 
                       help='Export data to Excel format (requires openpyxl or xlsxwriter)')
    parser.add_argument('--include_related_genes', action='store_true',
    ...
    group_A.add_argument("--group_A_str", help="Define Group A with a key-value string (e.g., 'cell_line=U937;treatment=DMSO').")
    group_A.add_argument("--group_A_file", help="Define Group A with a path to a query file.")
    
    # --- Group B Definition (Mutually Exclusive) ---
    group_B = parser.add_mutually_exclusive_group(required=True)
    group_B.add_argument("--group_B_str", help="Define Group B with a key-value string.")
    group_B.add_argument("--group_B_file", help="Define Group B with a path to a query file.")
    ...
    criteria_A = parse_criteria_string(args.group_A_str) if args.group_A_str else parse_criteria_file(args.group_A_file)
    criteria_B = parse_criteria_string(args.group_B_str) if args.group_B_str else parse_criteria_file(args.group_B_file)
    ...
    if not group_A_ids or not group_B_ids:
        print("ERROR: One or both of the defined groups resulted in zero samples. Exiting.", file=sys.stderr)
        sys.exit(1)
    ...
    raw_data = fetch_gene_data_raw_with_junction_context(con, args.gene, group_A_ids + group_B_ids)
    
    if not raw_data:
        con.close()
        print("ERROR: No data found for the specified gene or transcript ID. Exiting.", file=sys.stderr)
        sys.exit(1)
        
    # 4. Calculate all the aggregate statistics
    aggregated_data, norm_data = aggregate_replicate_data(raw_data.copy(), group_A_ids, group_B_ids)

    # 5. Add additional computed statistics
    aggregated_data=calculate_junction_confidence_intervals(aggregated_data,norm_data,n_boot=args.delta_psi_bootstrap)
    ...
    sample_int_to_string = dict(zip(norm_data['transcripts_long']['sample_id_int'].values,norm_data['transcripts_long']['sample_id_x'].values))
    figures = []
    sample_names = []
    for sid, sname in sample_int_to_string.items():
        figures.append(create_junction_penalty_heatmap_plotly(select_and_format_single_sample(raw_data, sid), sample_name=sname, output_prefix=None))
        sample_names.append(sname)
    
    # Save all individual sample penalty heatmaps to a single HTML with navigation
    penalty_heatmap_save_path = f"{args.output_dir}/{raw_data['gene_id']}_all_samples_penalty_heatmaps.html"
    ...
    if args.save_svg and STATIC_EXPORT_AVAILABLE:
        export_figures_as_svg_archive(figures, sample_names, f"{args.output_dir}/{raw_data['gene_id']}_all_samples_penalty_heatmaps.zip", raw_data['gene_id'])
        print(f"All individual sample penalty heatmaps saved to: {penalty_heatmap_save_path}")

    if args.save_pdf and HAVE_PYPDF2 and STATIC_EXPORT_AVAILABLE:
        pdf_save_path = f"{args.output_dir}/{raw_data['gene_id']}_all_samples_penalty_heatmaps.pdf"
    ...
    agg_heatmap = create_differential_junction_heatmap(aggregated_data,raw_data['gene_id'])
    agg_heatmap_save_path = f"{args.output_dir}/{raw_data['gene_id']}_aggregate_junction_heatmap.html"
    agg_heatmap.write_html(agg_heatmap_save_path)
    print(f"Aggregate junction heatmap saved to: {agg_heatmap_save_path}")
    if args.save_svg and STATIC_EXPORT_AVAILABLE:
        svg_save_path = f"{args.output_dir}/{raw_data['gene_id']}_aggregate_junction_heatmap.svg"
    ...
    if args.save_pdf and STATIC_EXPORT_AVAILABLE:
        pdf_save_path = f"{args.output_dir}/{raw_data['gene_id']}_aggregate_junction_heatmap.pdf"
    ...
    # Agg arc plot
    agg_arc_plot = create_junction_arc_plot_aggregated(aggregated_data, norm_data, raw_data['gene_id'], track_spacing=3)
    agg_arc_plot_save_path = f"{args.output_dir}/{raw_data['gene_id']}_aggregate_junction_arc_plot.html"
    agg_arc_plot.write_html(agg_arc_plot_save_path)
    print(f"Aggregate junction arc plot saved to: {agg_arc_plot_save_path}")
    if args.save_svg and STATIC_EXPORT_AVAILABLE:
        svg_save_path = f"{args.output_dir}/{raw_data['gene_id']}_aggregate_junction_arc_plot.svg"
    ...
    if args.save_pdf and STATIC_EXPORT_AVAILABLE:
        pdf_save_path = f"{args.output_dir}/{raw_data['gene_id']}_aggregate_junction_arc_plot.pdf"
    ...
    if len(norm_data['subset_links']) > 0:
        junction_figures = plot_all_subsets_junction_focus(aggregated_data, norm_data, show_individual_samples=True, include_territory_subsets=True)
        junction_save_path = f"{args.output_dir}/{raw_data['gene_id']}_all_subsets_junction_plots.html"
        export_multipanel_with_navigation(junction_figures, [fig.layout.title.text for fig in junction_figures], junction_save_path)
        print(f"All subset-focused junction plots saved to: {junction_save_path}")
        if args.save_svg and STATIC_EXPORT_AVAILABLE:
            export_figures_as_svg_archive(junction_figures, [fig.layout.title.text for fig in junction_figures], f"{args.output_dir}/{raw_data['gene_id']}_all_subsets_junction_plots.zip", raw_data['gene_id'])
        if args.save_pdf and HAVE_PYPDF2 and STATIC_EXPORT_AVAILABLE:
            pdf_save_path = f"{args.output_dir}/{raw_data['gene_id']}_all_subsets_junction_plots.pdf"
            create_combined_pdf(junction_figures, [fig.layout.title.text for fig in junction_figures], pdf_save_path)
    else:
        print("No subsets defined for this gene; skipping subset-focused junction plots.")
            
    # Agg subset-focused territory plots
    if len(aggregated_data['territories_agg']) > 1:
        territory_figures = plot_all_subsets_with_territories(aggregated_data, norm_data, show_individual_samples=True)
        territory_save_path = f"{args.output_dir}/{raw_data['gene_id']}_all_subsets_territory_plots.html"
        export_multipanel_with_navigation(territory_figures, [fig.layout.title.text for fig in territory_figures], territory_save_path)
        print(f"All subset-focused territory plots saved to: {territory_save_path}")
        if args.save_svg and STATIC_EXPORT_AVAILABLE:
            export_figures_as_svg_archive(territory_figures, [fig.layout.title.text for fig in territory_figures], f"{args.output_dir}/{raw_data['gene_id']}_all_subsets_territory_plots.zip", raw_data['gene_id'])
        if args.save_pdf and HAVE_PYPDF2 and STATIC_EXPORT_AVAILABLE:
            pdf_save_path = f"{args.output_dir}/{raw_data['gene_id']}_all_subsets_territory_plots.pdf"
            create_combined_pdf(territory_figures, [fig.layout.title.text for fig in territory_figures], pdf_save_path)     
    else:
    ...
    if args.export_excel:
        ...
        try:
            ...
            print(" ERROR: Excel export requested but required packages (openpyxl or xlsxwriter) are not installed.")
