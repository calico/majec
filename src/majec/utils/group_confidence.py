...
def process_sample_vectorized(args):
    ...
    """
    sample, sample_data, group_to_transcripts, aggregated_counts, gene_groups, te_groups, params = args
    ...
    valid_groups = [gid for gid in group_to_transcripts.keys() if gid in aggregated_counts.index]
    ...
    for group_id in valid_groups:
        ...
        for t in expressed_transcripts:
            if t in confidence_data:
                conf_scores.append((transcript_data.get(t, 0), confidence_data[t]['confidence_score']))
        ...
        if conf_scores:
            ...
            high_conf_fraction = np.sum(weights[high_conf_mask]) / np.sum(weights) if np.sum(weights) > 0 else 0
        else:
            weighted_conf = 0
            high_conf_fraction = 0
        ...
        group_weighted_unique_fraction = np.average(unique_fraction_scores, weights=expressed_values)
        group_weighted_shared_fraction = np.average(shared_fraction_scores, weights=expressed_values)
        ...
        group_weighted_dominant_reads_fraction = total_dominant_reads_in_group / aggregated_count
        ...
        for t_id, t_expr in zip(expressed_transcripts, expressed_values):
            ...
            abs_intergene_dist.append(t_metrics.get('ambiguous_fraction_abs_inter_dist', 1.0))
        ...
        if external_competitor_votes:
            ...
            main_external_competitor = max(weighted_counts.items(), key=lambda item: item[1])[0]
        ...
        group_total_distinguishability = np.average(dist_scores, weights=expressed_values)
        group_total_abs_intergene_dist = np.average(abs_intergene_dist, weights=expressed_values)
        ...
        has_junction = any(junction_data.get(t, {}).get('has_unique_junction_support', False) 
                          for t in expressed_transcripts)
        ...
        group_metrics[group_id] = {
            ...
            'strong_evidence_fraction': group_weighted_unique_fraction + group_weighted_dominant_reads_fraction,
            ...
            'holistic_group_distinguishability':(1 - group_weighted_shared_fraction) + ( group_weighted_shared_fraction * group_total_distinguishability),
            'holistic_group_external_distinguishability':(1 - group_weighted_shared_fraction) + ( group_weighted_shared_fraction * group_total_abs_intergene_dist),
...
def process_group_chunk(args):
    ...
    """
    chunk_id, group_chunk, sample, sample_data, aggregated_counts, gene_groups, te_groups, params = args
    ...
    _, group_metrics = process_sample_vectorized(
        (sample, sample_data, group_to_transcripts_chunk, aggregated_counts, gene_groups, te_groups, params)
    )
    
    return chunk_id, group_metrics

def save_outputs_parallel_optimized(all_sample_metrics, aggregated_counts, samples, 
                                   output_prefix, summary_stats, n_processes=4,
                                   compression_level=1):
    """
    ...
    column_order = ['sample', 'group_id'] + [col for col in comprehensive_df.columns 
                                             if col not in ['sample', 'group_id']]
...
def calculate_aggregated_confidence_metrics_parallel_optimized(
    ...
    **kwargs):
    ...
    mode_str = "Full"
    logging.info(f"=== Calculating Group-Level Confidence Metrics (Parallel Optimized, {mode_str} Mode) ===")
    ...
    logging.info(f"Processing {n_samples} samples × {n_groups} groups = {n_samples * n_groups:,} combinations")
    ...
    for i, sample in enumerate(samples):
        ...
        if isinstance(transcript_counts, pd.DataFrame):
            ...
            sample_data['transcript_data'] = data.to_dict() if isinstance(data, pd.Series) else data
        ...
        if isinstance(distinguishability_results, list) and i < len(distinguishability_results):
            sample_data['distinguishability_data'] = distinguishability_results[i]
        elif isinstance(distinguishability_results, dict):
            sample_data['distinguishability_data'] = distinguishability_results.get(sample, {})
        
        # Extract junction data
        if isinstance(junction_confidence_results, list) and i < len(junction_confidence_results):
    ...
    if n_samples == 1 and n_groups > 1000:
        ...
        logging.info(f"Single sample detected - using GROUP-PARALLEL strategy with {n_processes} processes")
        ...
        chunk_size = max(100, len(all_groups) // n_processes)  # At least 100 groups per chunk
        group_chunks = []
        for i in range(0, len(all_groups), chunk_size):
            group_chunks.append(all_groups[i:i + chunk_size])
        
        logging.info(f"Split {len(all_groups)} groups into {len(group_chunks)} chunks (~{chunk_size} groups/chunk)")
        
        # Create work items - one per chunk
        work_items = [
            (i, chunk, sample, sample_data, aggregated_counts, gene_groups, te_groups, params)
        ...
        logging.info(f"Using optimized sample-level processing with {min(n_processes, n_samples)} processes")
        
        # Create work items - one per sample
        work_items = [
            (sample, sample_data_dict[sample], group_to_transcripts, aggregated_counts, 
             gene_groups, te_groups, params)
    ...
    logging.info(f"Processed {len(samples) * n_groups:,} group-sample combinations in {processing_time:.1f}s "
    ...
    logging.info(f"Total time: {processing_time + io_elapsed:.1f}s "
...
def run_group_confidence_from_pipeline_parallel_optimized(
    ...
    te_map,
    args,
    int_to_string=None, string_to_int=None):
    """
    ...
    group_to_transcripts = all_mappings.groupby('AggregateID')['LocusID'].apply(list).to_dict()
    gene_groups = set(gene_map['AggregateID'].unique()) if gene_map is not None and not gene_map.empty else set()
