...
def decode_transcript_ids(data, int_to_string, copy=True):
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if df.index.dtype in ['int64', 'int32']:  # Integer index
            df.index = df.index.map(lambda x: int_to_string[x] if 0 <= x < len(int_to_string) else 'UNKNOWN')
        return df
    
    elif isinstance(data, pd.Series):
        s = data.copy() if copy else data
        if s.index.dtype in ['int64', 'int32']:  # Integer index
            s.index = s.index.map(lambda x: int_to_string[x] if 0 <= x < len(int_to_string) else 'UNKNOWN')
        s.name = data.name
        return s
        ...
        if first_key and isinstance(first_key, tuple):
            ...
            for feature_tuple, value in data.items():
                decoded_tuple = tuple(int_to_string[f] if 0 <= f < len(int_to_string) else 'UNKNOWN' 
            ...
            for feature_set, value in data.items():
                decoded_set = frozenset(int_to_string[f] if 0 <= f < len(int_to_string) else f'UNKNOWN_{f}'
...
def decode_dataframe_columns(df, int_to_string_map, columns_to_decode):
    ...
    def _decoder(item):
        ...
        if isinstance(item, (list, tuple, set, frozenset)):
            return type(item)(
                (int_to_string_map[i] if 0 <= i < len(int_to_string_map) else f"UNKNOWN_ID_{i}")
                for i in item
            )
            ...
            return {
                (int_to_string_map[k] if 0 <= k < len(int_to_string_map) else f"UNKNOWN_ID_{k}"): v
...
def generate_isoform_discrimination_report(junction_confidence_df, gene_to_transcripts_map, 
                                         junction_metrics, prefix, sample_name, int_to_string=None, junction_int_to_string=None):
    """
    ...
    for gene_id, transcript_list in gene_to_transcripts_map.items():
        ...
        transcripts_with_junctions = [t for t in transcript_list 
    ...
    if discrimination_reports:
        ...
        for report in discrimination_reports:
            summary_data.append({
                ...
                'n_discriminable_pairs': sum(1 for p in report['isoform_pairs'] if p['can_discriminate']),
        ...
        decode_transcript_ids(summary_df, int_to_string).to_csv(summary_output, sep='\t', index=False)
        
        logging.info(f"[{sample_name}] Generated isoform discrimination report for {len(discrimination_reports)} genes")
        ...
        if detailed_data:
            ...
            detailed_df = decode_transcript_ids(pd.DataFrame(detailed_data), int_to_string)
            ...
            if junction_int_to_string:
                ...
                detailed_df = decode_dataframe_columns(detailed_df, junction_int_to_string, columns_to_decode)

            # 3. Save the fully decoded DataFrame
            detailed_output = f"{prefix}_{sample_name}_isoform_discrimination_detailed.tsv.gz"
            detailed_df.to_csv(detailed_output, sep='\t', compression={'method': 'gzip', 'compresslevel': 1}, index=False)
...
def analyze_gene_isoform_discrimination(gene_id, transcript_list, junction_metrics):
    ...
    for t1, t2 in itertools.combinations(transcript_list, 2):
        ...
        discrimination_score = (len(unique_to_t1) + len(unique_to_t2)) / (len(shared) + 1)
...
def track_assignment_entropy_vectorized(unique_groups, multi_groups, current_priors, 
                                       min_expression=1.0, sample_name=None):
    """
    ...
    logging.info(f"[{sample_name}] Tracking entropy for {tracked_transcripts:,}/{total_transcripts:,} "
    ...
    for feature_tuple, read_count in unique_groups.items():
        if len(feature_tuple) == 1:
            ...
            if np.any(valid_mask):
                ...
                expressed_mask_local = np.array([f in expressed_transcripts for f in valid_features])
                if np.any(expressed_mask_local):
                    ...
                    if total_weight > 0:
                        ...
                        for feat, contrib, is_expr in zip(valid_features, contributions, expressed_mask_local):
                            if is_expr:
    ...
    logging.info(f"[{sample_name}] Entropy tracking completed in {elapsed:.1f} seconds (vectorized)")
    
    return dict(transcript_assignments)

def calculate_confidence_metrics(transcript_assignments, total_counts, #shared_fractions,
    ...
    for transcript, assignments in transcript_assignments.items():
        if transcript not in total_counts or total_counts[transcript] < min_count_threshold:
            continue
        ...
        if total_assigned > 0:
            ...
            unique_singleton_fraction = assignments.get('unique_singleton', 0) / total_assigned
            unique_multimapper_fraction = assignments.get('unique_multimapper', 0) / total_assigned
            # Derive the shared fraction. This is the only way to guarantee correctness.
            shared_frac = 1.0 - (unique_singleton_fraction + unique_multimapper_fraction)
...
def calculate_differential_confidence_vectorized(unique_mapper_groups, multimapper_classes, 
                                                current_priors, expressed_transcripts, 
                                                gene_map=None, te_map=None, sample_name=None, dominance_threshold = 0.5):
    """
    ...
    logging.info(f"[{sample_name}] Calculating distinguishability for {len(expressed_transcripts):,} "
    ...
    def _process_one_group_type(groups_dict):
        for feature_set, read_count in groups_dict.items():
            ...
            # Check expression
            expressed_mask = np.array([f in expressed_transcripts for f in valid_features])
            ...
            for i, (transcript, my_allocation) in enumerate(zip(expressed_features, expressed_allocations)):
                ...
                if my_allocation > dominance_threshold:
                    dominance_credit = (my_allocation - dominance_threshold) / (1 - dominance_threshold)
                ...
                all_transcript_data[transcript].append({
                    ...
                    'gene_allocations': dict(gene_allocations) if gene_allocations else None
                })
    
    logging.info(f"[{sample_name}] Processing {len(unique_mapper_groups)} unique mapper groups for distinguishability...")
    _process_one_group_type(unique_mapper_groups)
    
    logging.info(f"[{sample_name}] Processing {len(multimapper_classes)} multi-mapper groups for distinguishability...")
    ...
    for transcript in expressed_transcripts:
        ...
        for interaction in competitive_sets:
            ...
            weighted_avg_dist_sum += interaction['avg_distinguishability'] * class_weight
            weighted_min_dist_sum += interaction['min_distinguishability'] * class_weight
        ...
        # Final scores
        weighted_avg = weighted_avg_dist_sum / total_class_weight if total_class_weight > 0 else 1.0
        weighted_min = weighted_min_dist_sum / total_class_weight if total_class_weight > 0 else 1.0
        final_abs_inter_dist = weighted_abs_inter_dist_sum / total_class_weight if total_class_weight > 0 else 1.0
        
        # Final competitor analysis
        hardest_competitor_id, strongest_weight = max(competitor_weights.items(), key=lambda item: item[1]) if competitor_weights else (None, 0.0)
        total_comp_weight = sum(competitor_weights.values())
        intra_weight = sum(w for c, w in competitor_weights.items() if transcript_to_gene.get(c) == my_gene)
        frac_intra = intra_weight / total_comp_weight if total_comp_weight > 0 else 0.0
        frac_inter = 1.0 - frac_intra
        
        strongest_external_gene = max(external_gene_weights.items(), key=lambda item: item[1])[0] if external_gene_weights else None
        ...
        final_scores[transcript] = {
            ...
            'hardest_competitor_gene': transcript_to_gene.get(hardest_competitor_id, 'UNKNOWN'),
            'strongest_external_competitor_gene': strongest_external_gene,
            'hardest_competitor_weight_frac': strongest_weight / total_comp_weight if total_comp_weight > 0 else 0.0,
    ...
    if final_scores:
        comp_types = Counter(score['competition_type'] for score in final_scores.values())
        logging.info(f"[{sample_name}] Competition type breakdown:")
        for comp_type, count in sorted(comp_types.items()):
            logging.info(f"  - {comp_type}: {count} transcripts ({count/len(final_scores)*100:.1f}%)")
    
    elapsed = time.time() - start_time
    logging.info(f"[{sample_name}] Distinguishability calculation completed in {elapsed:.1f} seconds (vectorized)")
    
    return final_scores



def save_results_with_confidence(current_total_counts, confidence_metrics, 
                                       distinguishability_metrics, junction_confidence_metrics,
                                       prefix, sample_name, sample_pre_em_total_counts, verbose_output=False,
                                       full_feature_set=None, int_to_string=None):
    """
    ...
    n_total = len(current_total_counts) if full_feature_set is None else len(full_feature_set)
    
    logging.info(f"[{sample_name}] Processing confidence for {n_expressed:,} expressed features "
    ...
    if confidence_metrics:
        ...
        for col, fill_value in [('confidence_score', 0), ('entropy', np.inf), 
                               ('n_sources', 0), ('unique_fraction', 0)]:
    ...
    if distinguishability_metrics:
        ...
        # Add key columns
        key_dist_cols = ['distinguishability_score', 'n_unique_competitors', 'min_distinguishability',
                        'n_competitive_classes', 'hardest_competitor',
                        'competition_type', 'hardest_competitor_gene', 'dominant_reads', 'hardest_competitor_weight_frac', 
                        'ambiguous_fraction_abs_inter_dist','intra_gene_competition_frac', 'inter_gene_competition_frac' ]
        ...
        if 'confidence_score' in output_df_sparse.columns and 'distinguishability_score' in output_df_sparse.columns:
            unique_mappers = output_df_sparse['confidence_score'] == 1.0
            output_df_sparse.loc[unique_mappers, 'distinguishability_score'] = \
                output_df_sparse.loc[unique_mappers, 'distinguishability_score'].fillna(2.0)
            output_df_sparse['distinguishability_score'] = output_df_sparse['distinguishability_score'].fillna(-1)
        ...
        if verbose_output and not dist_df.empty:
            ...
            if len(dist_expressed) > 0:
                ...
                # Save sparse version
                output_file = f"{prefix}_{sample_name}_distinguishability_detailed.tsv.gz"
                dist_output_df.to_csv(output_file, sep='\t', 
                                    compression={'method': 'gzip', 'compresslevel': 1})
                logging.info(f"[{sample_name}] Saved detailed distinguishability for "
    ...
    if junction_confidence_metrics:
        ...
        key_junction_cols = ['junction_evidence_fraction', 'has_unique_junction_support', 
        ...
        if 'junction_evidence_fraction' in output_df_sparse.columns:
            output_df_sparse['junction_evidence_fraction'] = output_df_sparse['junction_evidence_fraction'].fillna(0)
        if 'has_unique_junction_support' in output_df_sparse.columns:
            output_df_sparse['has_unique_junction_support'] = junc_df['has_unique_junction_support'].astype('boolean')
            output_df_sparse['has_unique_junction_support'] = output_df_sparse['has_unique_junction_support'].fillna(False)
        if 'junction_confidence_category' in output_df_sparse.columns:
            output_df_sparse['junction_confidence_category'] = output_df_sparse['junction_confidence_category'].fillna('no_junctions')
    ...
    # Add discord scores
    output_df_sparse['discord_score']=calculate_junction_read_discord(sample_pre_em_total_counts, current_total_counts)

    # Add overall confidence category
    output_df_sparse['overall_confidence'] = categorize_overall_confidence(output_df_sparse)
    ...
    if 'distinguishability_score' in output_df_sparse.columns:
        ...
        output_df_sparse['dominant_fraction'] = output_df_sparse['dominant_reads'] / output_df_sparse['count']
        if 'confidence_score' in output_df_sparse.columns:
            output_df_sparse['strong_evidence_fraction'] = output_df_sparse['dominant_fraction'].fillna(0.0) + output_df_sparse['unique_fraction'].fillna(0.0) + output_df_sparse['unique_multimapper_fraction'].fillna(0.0)
            output_df_sparse['strong_evidence_fraction'] = output_df_sparse['strong_evidence_fraction'].clip(0, 1)
    ...
    if 'entropy' in output_df_sparse.columns:
        finite_entropy = output_df_sparse['entropy'][output_df_sparse['entropy'] != np.inf]
    ...
    if full_feature_set is not None:
        ...
        output_df_sparse.rename(columns={'distinguishability_score': 'ambiguous_fraction_distinguishability'}, inplace=True)
        is_unique_only = output_df_sparse['shared_read_fraction'] == 0
        output_df_sparse.loc[is_unique_only, 'ambiguous_fraction_distinguishability'] = 1.0
        output_df_sparse['distinguishability_score'] = ((1 - output_df_sparse['shared_read_fraction']) * 1.0 +
            (output_df_sparse['shared_read_fraction'] * output_df_sparse['ambiguous_fraction_distinguishability']))
        output_df_sparse['abs_inter_dist'] = ((1 - output_df_sparse['shared_read_fraction']) * 1.0 + 
            (output_df_sparse['shared_read_fraction'] * output_df_sparse['ambiguous_fraction_abs_inter_dist']))
        ...
        if 'has_unique_junction_support' in output_df_full.columns:
            output_df_full['has_unique_junction_support'] = output_df_full['has_unique_junction_support'].astype(bool)
    ...
    if not output_df_sparse.empty:
        ...
        decode_transcript_ids(output_df_sparse, int_to_string).to_csv(
            metrics_file,
            compression={'method': 'gzip', 'compresslevel': 1}, sep='\t'
        )
        logging.info(f"[{sample_name}] Saved sparse metrics for DB ingestion to {metrics_file}")

    # Save outputs
    if verbose_output:
        # Only sort the sparse version for speed
        output_df_sorted = decode_transcript_ids(output_df.sort_values('count', ascending=False), int_to_string)
        output_df_sorted.to_csv(f"{prefix}_{sample_name}_counts_with_confidence.tsv.gz", 
                              compression={'method': 'gzip', 'compresslevel': 1}, sep='\t')
    ...
    if high_conf_mask.any():
        ...
        logging.warning(f"[{sample_name}] No transcripts met high-confidence criteria. "
...
def analyze_competition_patterns(distinguishability_results, output_prefix, int_to_string=None):
    """
    ...
    summary = df.groupby(['competition_type']).agg({
        'distinguishability': ['mean', 'median', 'std', 'count']
    }).round(3)
    
    decode_transcript_ids(summary, int_to_string).to_csv(f"{output_prefix}_competition_summary.tsv", sep='\t')
    
    # Identify genes with complex competition
    gene_competition = df.groupby('gene')['competition_type'].value_counts().unstack(fill_value=0)
    complex_genes = gene_competition[gene_competition.sum(axis=1) > 10].sort_values(
        by='intra-genic', ascending=False
    )
    
    decode_transcript_ids(complex_genes, int_to_string).to_csv(f"{output_prefix}_complex_competition_genes.tsv", sep='\t')
    
    logging.info(f"Competition analysis saved to {output_prefix}_competition_summary.tsv")
...
def categorize_overall_confidence(df):
    ...
    if has_entropy and has_dist and has_junction and has_sharing:
        conditions.append(
            ...
            ((df['distinguishability_score'] > 0.8) | (df['has_unique_junction_support'].astype(bool))) &
    ...
    return pd.Series(np.select(conditions_array, choices, default='low'), index=df.index)
...
def add_shared_read_metrics(df, unique_groups, multi_groups):
    ...
    for transcript in df.index:
        unique_reads = sum(count for group, count in unique_groups.items() 
                          if len(group) == 1 and group[0] == transcript)
        
        shared_reads = sum(count for group, count in unique_groups.items()
                          if transcript in group and len(group) > 1)
        shared_reads += sum(count for group, count in multi_groups.items()
    ...
    df['adjusted_distinguishability'] = df['distinguishability_score'] * (1 - df['shared_read_fraction'])
...
def process_and_save_sample_outputs(args_tuple):
    ...
    """
    (sample_name, sample_counts, sample_confidence, sample_distinguishability, 
     sample_junction_confidence, sample_junction_metrics, sample_pre_em_total_counts, gene_to_transcripts_map, 
     prefix, verbose_output, 
     int_to_string, junction_int_to_string) = args_tuple

    logging.info(f"[{sample_name}] Saving comprehensive confidence and junction reports...")
    ...
    if sample_junction_confidence:
        ...
        if 'has_unique_junction_support' in junc_df.columns:
            ...
            if 'junction_specificity_score' in junc_df.columns:
                specific_mask = (~unique_mask) & (junc_df['junction_specificity_score'] > 0)
                junction_support[specific_mask] = 0.5 + (0.4 * junc_df.loc[specific_mask, 'junction_specificity_score'])
                
                if 'n_shared_junctions' in junc_df.columns:
                    ambiguous_mask = (~unique_mask) & (junc_df['junction_specificity_score'] <= 0) & (junc_df['n_shared_junctions'] > 0)
    ...
    if 'distinguishability_score' in output_df.columns:
        summary_data[f'{sample_name}_distinguishability'] = output_df['distinguishability_score']
...
def track_assignment_entropy_parallel(unique_groups, multi_groups, current_priors, 
                                    min_expression=1.0, threads_per_sample=1, sample_name=None):
    """
    ...
    if threads_per_sample > 1:
        logging.debug(f"Ignoring threads_per_sample={threads_per_sample}, using vectorized implementation")
    
    return track_assignment_entropy_vectorized(unique_groups, multi_groups, current_priors, min_expression, sample_name=sample_name)


def calculate_differential_confidence_parallel(unique_mapper_groups, multimapper_classes, 
                                             current_priors, expressed_transcripts, 
                                             gene_map=None, te_map=None,
                                             threads_per_sample=1, sample_name=None):
    """
    Wrapper that maintains the parallel interface but uses vectorized implementation.
    The threads_per_sample parameter is ignored since vectorization is more efficient.
    """
    ...
    return calculate_differential_confidence_vectorized(
        unique_mapper_groups, multimapper_classes, current_priors, 
        expressed_transcripts, gene_map, te_map, sample_name=sample_name
    )
