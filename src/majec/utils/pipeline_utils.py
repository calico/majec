...
def load_precomputed_junction_map(pickle_file):
    ...
    logging.info(
        f"Loaded junction map with {data['metadata']['total_junctions']:,} junctions"
    )
    logging.info(
        f"  ({data['metadata']['unique_junctions']:,} unique, "
...
def calculate_effective_length_distributional(transcript_lengths, frag_mean, frag_sd):
    ...
    try:
        ...
        logging.warning(
            "scipy not available - using approximate truncated normal", RuntimeWarning
        )
...
def create_convergence_groups(pipeline_context, theta, percentile_thresholds=[25, 75]):
    ...
    if pipeline_context["global_thresholds"] is None:
        logging.warning(
            "Global expression thresholds not calculated! Using fallback per-sample binning."
        )
        ...
        low_threshold = (
            np.percentile(theta[theta > 0], percentile_thresholds[0])
            if np.sum(theta > 0) > 0
            else 1.0
        )
        high_threshold = (
            np.percentile(theta[theta > 0], percentile_thresholds[1])
            if np.sum(theta > 0) > 0
            else 100.0
        )
    else:
...
def grouped_momentum_acceleration(
    pipeline_context,
    theta_history,
    unique_groups,
    multi_groups,
    previous_multi_counts,
    base_momentum=0.3,
    momentum_scaling_values=[1.5, 1.0, 0.7],
):

    momentum_scaling = {
        "low": momentum_scaling_values[0],
    ...
    if len(theta_history) < 3:
        ...
        return standard_em_step(
            theta_history[-1], unique_groups, multi_groups, previous_multi_counts
        )
    ...
    try:
        ...
        for group_name, indices in groups.items():
            if len(indices) > 0:
                ...
                velocity_correlation = (
                    np.corrcoef(group_velocity, group_prev_velocity)[0, 1]
                    if len(indices) > 1
                    else 1.0
                )
                ...
                base_group_momentum = base_momentum * momentum_scaling.get(
                    group_name, 1.0
                )
        ...
        relative_change = np.abs(theta_momentum - current_theta) / (
            current_theta + 1e-8
        )
        ...
        if max_change > 1.0:
            return standard_em_step(
                current_theta, unique_groups, multi_groups, previous_multi_counts
            )

        # Use EM step for final decomposition
        _, momentum_unique, momentum_multi = standard_em_step(
            theta_momentum, unique_groups, multi_groups, previous_multi_counts
        )
...
def adaptive_convergence_check(
    current_total,
    previous_total,
    iteration,
    min_iterations=15,
    ...
    total_change = abs(current_total.sum() - previous_total.sum()) / (
        previous_total.sum() + 1e-6
    )
    ...
    if meaningful_mask.sum() > 0:
        ...
        feature_changes = abs(meaningful_current - meaningful_previous) / (
            meaningful_previous + 1e-6
        )
        ...
        logging.info(
            f"     Meaningful features: {meaningful_mask.sum():,}/{len(current_total):,} "
            f"(threshold≥{adaptive_threshold:.1f})"
        )

        # Convergence criteria
        converged = (
            (total_change < relative_threshold)
            and (max_feature_change < relative_threshold * 5)  # Less strict than 10
            and (mean_feature_change < relative_threshold * 0.5)  # More strict than 1
            and (p95_feature_change < relative_threshold * 2)  # Less strict than 3
        )

        if converged:
            logging.info(f"     ✅ CONVERGENCE ACHIEVED: All criteria satisfied")
            logging.info(
                f"       Final state: {meaningful_mask.sum()} meaningful features, "
