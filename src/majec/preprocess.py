...
try:
    ...
    logging.error(f"Failed to import 'intervaltree'. Can not generate_rescue_features . Error: {e}")
    ...
    try:
        ...
        with open(file_path, 'r') as f:
            for i, line in enumerate(f):
                ...
                # --- The New Scrutiny ---
                if len(parts) < 1:
                    logging.warning(f"Skipping malformed line {i+1} in {file_path}: line is empty.")
                    continue
                ...
                if ' ' in chrom_name or '\t' in chrom_name:
                    logging.warning(f"Chromosome name '{chrom_name}' on line {i+1} contains whitespace. This may cause issues.")
        ...
        logging.info(f"Successfully loaded and validated {len(chrom_order)} chromosomes from {os.path.basename(file_path)}.")
        return chrom_order
...
def parse_complete_gtf(gtf_path, feature_type='gene'):
    ...
    with open(gtf_path, 'r') as f:
        for line in f:
            ...
            if feature == 'exon':
                ...
                transcript_info[transcript_id]['start'] = min(transcript_info[transcript_id]['start'], start)
                transcript_info[transcript_id]['end'] = max(transcript_info[transcript_id]['end'], end)
    ...
    logging.info(f"  Parsed {len(transcript_info)} transcripts in {len(gene_to_transcripts)} genes/families")
...
def compute_junction_data(transcript_to_exons, string_to_int):
    ...
    for transcript_id, exons in transcript_to_exons.items():
        ...
        for i in range(len(exons_sorted) - 1):
            ...
            junction_key = f"{exon1['chr']}:{exon1['end']}-{exon2['start']}:{exon1['strand']}"
    ...
    for junction_key, transcript_ints in junction_to_transcripts.items():
        ...
        junction_map[junction_key] = {
            'transcripts': list(transcript_ints),  # Note: 'transcripts' not 'transcript_ids'
            'is_unique': is_unique,
...
def calculate_transcript_expected_junctions(transcript_to_exons, string_to_int):
    ...
    # Log statistics
    n_single_exon = sum(1 for v in transcript_expected_junctions.values() if v['n_expected'] == 0)
    n_multi_exon = len(transcript_expected_junctions) - n_single_exon
    avg_junctions = np.mean([v['n_expected'] for v in transcript_expected_junctions.values()])
...
def create_transcript_junction_order(junction_map):
    ...
    for transcript_id, junctions in transcript_to_junctions.items():
        ...
        try:
            ...
            print(f"Warning: Could not parse and sort junctions for transcript {transcript_id}")
            continue
...
def process_region_for_subsets(args):
    ...
    for subset_tid, info in subset_relationships.items():
        ...
        max_superset_junctions = max(len(transcript_junction_sets[sid]) 
                                    for sid in info['supersets'])
...
def identify_subset_superset_relationships_parallel(junction_map, transcript_info, n_cores=6, 
                                                  bin_size=50_000_000, overlap=10_000_000):
    """
    ...
    for bin_key, transcript_ids in bins.items():
        if len(transcript_ids) >= 2:
            region_junction_sets = {
                tid: transcript_junction_sets[tid] 
    ...
    for region_subsets in region_results:
        for subset_tid, relationship in region_subsets.items():
            if subset_tid not in all_subset_relationships:
                ...
                existing_supersets = set(all_subset_relationships[subset_tid]['supersets'])
                new_supersets = set(relationship['supersets'])
                all_subset_relationships[subset_tid]['supersets'] = list(existing_supersets | new_supersets)
    ...
    for subset_tid, info in all_subset_relationships.items():
        ...
        max_superset_junctions = max(
            len(transcript_junction_sets[sid]) 
            for sid in info['supersets']
        )
        info['coverage_fraction'] = n_subset_junctions / max_superset_junctions
        info['unique_junction_deficit'] = max_superset_junctions - n_subset_junctions
...
def characterize_unique_exonic_territory_parallel(
    ...
    """
    logging.info(f"Characterizing {len(subset_relationships)} subsets in parallel with {num_workers} workers...")
    ...
    with ProcessPoolExecutor(
        ...
        initargs=(subset_relationships, transcript_to_exons, int_to_string, min_unique_size)
    ) as executor:
    ...
    logging.info(f"  Found unique territories for {len(final_map)}/{len(subset_relationships)} subsets")
...
def _worker_characterize_chunk(subset_chunk):
    ...
    subset_relationships = _worker_data['subset_relationships'] # The full map is needed for context
    ...
    for subset_id in subset_chunk:
        ...
        for superset_id in details['supersets']:
            ...
            overlapping_intervals = superset_tree.overlap(subset_first_exon['start'], subset_first_exon['end'])
            
            if overlapping_intervals:
                superset_comparator_interval = min(overlapping_intervals, key=lambda iv: iv.begin)
                superset_comparator = superset_comparator_interval.data

                if subset_first_exon['start'] < superset_comparator['start']:
                    unique_region_size = superset_comparator['start'] - subset_first_exon['start']
            ...
            overlapping_intervals = superset_tree.overlap(subset_last_exon['start'], subset_last_exon['end'])

            if overlapping_intervals:
                superset_comparator_interval = max(overlapping_intervals, key=lambda iv: iv.end)
                superset_comparator = superset_comparator_interval.data

                if subset_last_exon['end'] > superset_comparator['end']:
                    unique_region_size = subset_last_exon['end'] - superset_comparator['end']
                    if unique_region_size >= min_unique_size:
                        unique_territories.append({
            ...
            for interval in subset_unique:
                if (interval.begin > subset_exons_sorted[0]['start'] + 100 and
                    interval.end < subset_exons_sorted[-1]['end'] - 100):
                    
                    upstream_exon, downstream_exon = None, None
                    for sup_exon in superset_exons_sorted:
                        if sup_exon['end'] <= interval.begin:
                            if not upstream_exon or sup_exon['end'] > upstream_exon['end']:
                                upstream_exon = sup_exon
                        elif sup_exon['start'] >= interval.end:
                            if not downstream_exon or sup_exon['start'] < downstream_exon['start']:
                                downstream_exon = sup_exon
                    
                    if upstream_exon and downstream_exon and interval.length() >= min_unique_size:
                        comparator_coords = [
...
def build_interval_trees(transcript_to_exons, string_to_int):
    ...
    for transcript_id_str, exons in transcript_to_exons.items():
        ...
        #print(chrom, strand)
        for exon in exons:
            if exon['start'] != exon['end']: #safty check for zero-length exons could potentially filter GTF vecotwise earlier
                ...
                trees[chrom][strand].add(Interval(exon['start'], exon['end'], transcript_id_int))


    logging.info(f"Successfully built and merged interval trees for {len(trees)} chromosomes.")
    return trees

def find_unambiguous_territory(
    candidate_map, 
    interval_trees, 
    transcript_info
):
    ...
    """
    logging.info(f"Searching for unambiguous 'daylight' in {len(candidate_map)} candidate subsets...")
    ...
    for subset_id, events in candidate_map.items():
        ...
        for event in events:
            try:
                ...
                overlapping_intervals = interval_trees[chrom][strand].overlap(start, end)
                ...
                if daylight_tree:
                    ...
                    largest_daylight_interval = max(daylight_tree, key=lambda i: i.length())
                    
                    # We create a new, validated event with the precise coordinates of the daylight.
                    updated_event = event.copy()
                    updated_event['unique_coords'] = f"{chrom}:{largest_daylight_interval.begin}-{largest_daylight_interval.end}"
                    updated_event['size'] = largest_daylight_interval.length() # Update the size
                    
                    validated_events.append(updated_event)

            except Exception as e:
                # This robustly handles any unexpected errors for a single event.
                logging.warning(f"Could not process event for subset {subset_id} ({event.get('unique_coords', 'N/A')}): {e}")
                continue
    ...
    logging.info(f"Found {len(final_rescue_map)} subsets with validated, unambiguous territory.")
...
def write_territories_to_bed_with_mapping(
    ...
    if chrom_order is None or len(chrom_order) == 0:
        logging.warning("Could not determine chromosome order from GTF header. Falling back to simple sort.")
    ...
    territory_mapping = {
        ...
        'subset_territories': defaultdict(lambda: {'unique': [], 'comparator': []}),  # subset_id -> {unique: [region_ids], comparator: [region_ids]}
    ...
    for subset_id, events in final_rescue_map.items():
        ...
        for event in events:
            ...
            try:
                ...
                if region_id not in territory_mapping['subset_territories'][subset_id]['unique']:
                    territory_mapping['subset_territories'][subset_id]['unique'].append(region_id)
            ...
            for coords_str in comparator_list:
                try:
                    ...
                    region_id = unique_regions[region_key]
                    if region_id not in territory_mapping['subset_territories'][subset_id]['comparator']:
                        territory_mapping['subset_territories'][subset_id]['comparator'].append(region_id)
    ...
    territory_mapping['subset_territories'] = dict(territory_mapping['subset_territories'])
...
def build_comprehensive_annotation(gene_gtf, te_gtf, output_prefix, rescue_data=True, genome_file_path=None):
    """Build complete annotation data structure with TSL and expected junctions."""
    
    # Parse GTFs (now includes TSL)
    gene_transcript_info, gene_to_transcripts, gene_transcript_exons, gene_chroms = parse_complete_gtf(gene_gtf, 'gene')
    ...
    te_chroms= []
    if te_gtf:
        te_transcript_info, te_to_transcripts, te_transcript_exons, te_chroms = parse_complete_gtf(te_gtf, 'te')
    ...
    if rescue_data:
        ...
        chroms_in_gtf_but_not_genome = all_gtf_chroms.difference(authoritative_chrom_set)
        if chroms_in_gtf_but_not_genome:
            ...
            logging.warning(f"The following {len(chroms_in_gtf_but_not_genome)} chromosome(s) were found in your GTF file(s) but are NOT present")
            logging.warning(f"in your authoritative genome file ('{os.path.basename(genome_file_path)}').")
            logging.warning("Any transcripts or features on these chromosomes will be effectively ignored by the aligner")
            logging.warning("and downstream tools like bedtools. This can lead to unexpected results.")
            ...
            logging.warning("Please ensure your GTF and genome files are derived from the exact same reference assembly.")
            logging.warning("="*80)
        else:
            logging.info("✓ Chromosome name consistency check passed. All GTF chromosomes are present in the genome file.")
    ...
    subset_relationships = identify_subset_superset_relationships_parallel(junction_map, transcript_info_int)
    ...
    junction_string_to_int, junction_int_to_string = create_integer_mappings(master_junction_set)
    ...
    for tid, data in subset_relationships.items():
        subset_relationships_int[tid] = {
            ...
            'shared_junctions': [junction_string_to_int[j] for j in data['shared_junctions']],
    ...
    junction_string_to_int[k]: {junction_string_to_int[v] for v in competitor_set} # <-- Changed to square brackets
    for k, competitor_set in junction_competitors.items()
}
    logging.info("  Conversion complete.")

    logging.info(f"  Found {len(master_junction_set):,} unique junction strings to be encoded.")
    # Calculate checksums
    ...
    if rescue_data:
        logging.info("Starting process to identify unique exonic territories and comparator regions...")        
        ...
        if candidate_map:
            logging.info(f"  Identified {len(candidate_map)} candidate subsets with potential unique territory.")
            ...
            logging.info("  Performing definitive overlap checking to confirm unambiguous territory...")
            ...
            if final_rescue_map:
                logging.info(f"  Confirmed {len(final_rescue_map)} subsets with unambiguous unique territory after overlap checking.")
                # Write to bed
                subset_coverage_bed_path = f"{output_prefix}_subset_coverage_features.bed"
                subset_coverage_territory_mapping = write_territories_to_bed_with_mapping(final_rescue_map, transcript_info_int, subset_coverage_bed_path, chrom_order)
            else:
                logging.info("  No subsets confirmed with unambiguous unique territory after overlap checking.")
        else:
            logging.info("  No candidate subsets identified with potential unique territory.")
    else:
        logging.info("Skipping unique exonic territory and comparator region generation as per user request.")  
    ...
    annotation_data = {
        ...
        'te_map': {
            ...
            'AggregateID': te_map['AggregateID'].values if te_map is not None else None
        } if te_map is not None else None,
        ...
        'subset_coverage_features': final_rescue_map if rescue_data and final_rescue_map else None,
        'subset_coverage_territory_mapping': subset_coverage_territory_mapping if rescue_data and final_rescue_map else None,
        # Metadata
        'metadata': {
            'gene_gtf': os.path.abspath(gene_gtf),
            'te_gtf': os.path.abspath(te_gtf) if te_gtf else None,
            'subset_coverage_features_bed_path': os.path.abspath(subset_coverage_bed_path) if rescue_data and final_rescue_map else None,
            'genome_file_path': os.path.abspath(genome_file_path) if genome_file_path else None,
            ...
            'n_unique_junctions': sum(1 for j in junction_map.values() if j['is_unique']),
    ...
    summary = {
        ...
        'statistics': {
            ...
            'expected_junctions': {
                'single_exon_transcripts': sum(1 for v in transcript_expected_junctions.values() if v['n_expected'] == 0),
                'multi_exon_transcripts': sum(1 for v in transcript_expected_junctions.values() if v['n_expected'] > 0),
                'avg_junctions_per_transcript': np.mean([v['n_expected'] for v in transcript_expected_junctions.values()])
            },
            'unique_junctions_per_transcript': {
                'mean': np.mean([len(j) for j in transcript_unique_junctions.values()]) if transcript_unique_junctions else 0,
                'max': max([len(j) for j in transcript_unique_junctions.values()]) if transcript_unique_junctions else 0,
...
def main():
    ...
    parser.add_argument('--gene_gtf', required=True, 
                        help='Gene annotation GTF file')
    parser.add_argument('--te_gtf', 
                        help='TE annotation GTF file (optional)')
    parser.add_argument('--output', default='majec', 
                        help='Output prefix for generated files')
    parser.add_argument('--validate', 
                        help='Validate existing annotation file')
    parser.add_argument('--generate_rescue_features', action='store_true',
    ...
    parser.add_argument('--genome_file_path', 
    ...
    if args.generate_rescue_features and not args.genome_file_path:
        parser.error("--genome_file_path is required when --generate_rescue_features is set.")
        sys.exit(1)
    ...
    annotation_data = build_comprehensive_annotation(
        args.gene_gtf, args.te_gtf, args.output, args.generate_rescue_features, args.genome_file_path
    )
