...
FINAL_COUNTS_SCHEMA = [
    {"name": "sample_id_int", "dtype": "INTEGER"},
    {"name": "transcript_id_int", "dtype": "INTEGER"},
    {"name": "count", "dtype": "REAL"},
    {"name": "tpm", "dtype": "REAL"},
]
...
DISTINGUISHABILITY_SCHEMA = [
    {"name": "sample_id_int", "dtype": "INTEGER"},
    {"name": "transcript_id_int", "dtype": "INTEGER"},
    {"name": "distinguishability_score", "dtype": "REAL"},
    {"name": "min_distinguishability", "dtype": "REAL"},
    {"name": "n_competitive_classes", "dtype": "INTEGER"},
    {"name": "n_unique_competitors", "dtype": "INTEGER"},
    {"name": "hardest_competitor_int", "dtype": "INTEGER"},
    {"name": "hardest_competitor_gene", "dtype": "TEXT"},
    {"name": "competition_type", "dtype": "TEXT"},
    {"name": "my_gene", "dtype": "TEXT"},
    {"name": "transcript_type", "dtype": "TEXT"},
    {"name": "count", "dtype": "REAL"},
]

GROUP_CONFIDENCE_SCHEMA = [
    {"name": "sample_id_int", "dtype": "INTEGER"},
    {"name": "group_id", "dtype": "TEXT"},
...
def connect_db(db_path, force=False):
    ...
    if os.path.exists(db_path) and not force:
        print(
            f"WARNNING: Database '{db_path}' already exists. Use --force to overwrite.",
            file=sys.stderr,
        )
        # sys.exit(1)
...
def extract_manifest_metadata(con, cur, sample_names_from_manifest):
    ...
    """
    print(
        "creating minimal sample metadata from manifest. no downstream statistics will be enabled ..."
    )
...
def merge_manifests(manifest_paths):
    ...
    for idx, manifest_path in enumerate(manifest_paths):
        ...
        overlap_cache = set(merged["cache_files"].keys()) & set(
            manifest.get("cache_files", {}).keys()
        )
        if overlap_cache:
            print(
                f"ERROR: Sample overlap detected in cache files: {overlap_cache}",
                file=sys.stderr,
            )
            sys.exit(1)

        # Check for sample overlaps in per-sample files
        overlap_per_sample = set(merged["per_sample_files"].keys()) & set(
            manifest.get("per_sample_files", {}).keys()
        )
        if overlap_per_sample:
            print(
                f"ERROR: Sample overlap detected in per-sample files: {overlap_per_sample}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        print(
            f"    Added {len(manifest.get('cache_files', {}))} samples from this manifest"
        )
    ...
    print(
        f"Merge complete: {total_samples} total samples from {len(manifest_paths)} manifests"
    )
    return merged
...
def parse_and_ingest_metadata(db_path, metadata_file, sample_names_from_manifest):
    ...
    if not experimental_vars:
        print(
            "WARNING: No '#experimental_variables:' line found in metadata file. Cannot perform statistical comparisons.",
            file=sys.stderr,
        )
    ...
    try:
        ...
        print(
            f"ERROR: Failed to parse the data table in '{metadata_file}': {e}",
            file=sys.stderr,
        )
        return False
    ...
    if missing_from_metadata:
        print(
            "\nFATAL ERROR: The following samples were found in the MAJEC run but are MISSING from your metadata file:",
            file=sys.stderr,
        )
    ...
    print(
        "  -> Validation successful: All run samples are present in the metadata file."
    )
    ...
    if df_meta.columns[0] != "sample_id":
        print(
            f"ERROR: The first column in the metadata file must be named 'sample_id'. Found '{df_meta.columns[0]}' instead.",
            file=sys.stderr,
        )
        return False
    ...
    all_data_columns = set(
        df_meta.columns[1:]
    )  # All columns except the first ID column
    if not all_declared_vars.issubset(all_data_columns):
        missing_cols = all_declared_vars - all_data_columns
        print(
            f"ERROR: The following variables were declared in the header but are missing as columns in the data: {missing_cols}",
            file=sys.stderr,
        )
        return False
...
def ingest_annotation_data(con, cur, anno_data):
    ...
    df_transcripts.to_sql(
        "transcript_annotations", con, if_exists="replace", index=False
    )
    del df_transcripts
    ...
    if transcript_junction_links:
        cur.executemany(
            "INSERT INTO transcript_junctions VALUES (?, ?, ?)",
            transcript_junction_links,
        )
    ...
    if junction_records:
        ...
        df_junctions.to_sql(
            "junction_annotations", con, if_exists="replace", index=False
        )
    ...
    if string_to_int_map:
        ...
        df_transcript_map.to_sql(
            "transcript_id_map", con, if_exists="replace", index=False
        )
...
def ingest_territory_data(con, cur, anno_data):
    ...
    print(
        f"  Processing links for {len(territory_mapping['subset_territories'])} transcripts..."
    )
    link_records = []
    ...
    if link_records:
        cur.executemany(
            "INSERT INTO transcript_territories VALUES (?, ?, ?)", link_records
        )
...
def ingest_matrix_data_from_multiple_files(
    con,
    cur,
    table_name,
    counts_files,
    ...
    if len(counts_files) != len(tpm_files):
        print(
            f"ERROR: Mismatch in number of counts ({len(counts_files)}) and TPM ({len(tpm_files)}) files",
            file=sys.stderr,
        )
        sys.exit(1)

    print(
        f"--- Ingesting {len(counts_files)} matrix file pairs for table: {table_name} ---"
    )
...
def ingest_matrix_data_by_column_batches(
    cur,
    table_name,
    counts_file,
    tpm_file,
    ...
    if set(sample_columns_counts) != set(sample_columns_tpm):
        print(
            "FATAL ERROR: Sample columns in counts and TPM files do not match.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    for i in range(0, len(sample_columns), column_batch_size):
        ...
        print(
            f"  Processing samples {i+1} to {min(i+column_batch_size, len(sample_columns))} of {len(sample_columns)}..."
        )
        ...
        df_counts_batch = pl.read_csv(
            counts_file, separator="\t", columns=counts_cols_to_read
        )
        df_tpm_batch = pl.read_csv(tpm_file, separator="\t", columns=tpm_cols_to_read)

        if is_aggregated:
            df_counts_batch = df_counts_batch.select(
                [
                    pl.col(df_counts_batch.columns[0]).alias("group_id"),
                    *[pl.col(c) for c in df_counts_batch.columns[1:]],
                ]
            )
            df_tpm_batch = df_tpm_batch.select(
                [
                    pl.col(df_tpm_batch.columns[0]).alias("group_id"),
            ...
            df_counts_batch = df_counts_batch.rename(
                {df_counts_batch.columns[0]: "transcript_id_str"}
            )
            df_counts_batch = df_counts_batch.join(
                transcript_lookup, on="transcript_id_str", how="left"
            ).drop("transcript_id_str")

            df_tpm_batch = df_tpm_batch.rename(
                {df_tpm_batch.columns[0]: "transcript_id_str"}
            )
            df_tpm_batch = df_tpm_batch.join(
                transcript_lookup, on="transcript_id_str", how="left"
            ).drop("transcript_id_str")
        ...
        df_counts_long = df_counts_batch.melt(
            id_vars=[id_col], variable_name="sample_id", value_name="count"
        )
        df_tpm_long = df_tpm_batch.melt(
            id_vars=[id_col], variable_name="sample_id", value_name="tpm"
        )
        ...
        df_long = df_long.with_columns(
            [
                ...
                pl.col("tpm").cast(pl.Float64, strict=False),
            ]
        )
        ...
        df_long = df_long.with_columns(
            pl.col("sample_id").replace(sample_map).alias("sample_id_int")
        )
...
def ingest_per_sample_data(
    ...
    try:
        ...
        create_table_sql = (
            f"CREATE TABLE {table_name} ({', '.join(final_table_columns)})"
        )
    ...
    with con:
        for file_idx, (sample_name, paths_dict) in enumerate(per_sample_map.items()):
            ...
            if not file_path or not os.path.exists(file_path):
                continue
            print(
                f"  Processing file {file_idx + 1}/{len(per_sample_map)}: {os.path.basename(file_path)}..."
            )
            ...
            final_db_columns_for_select = [
                "sample_id_int",
                "transcript_id_int",
            ] + column_names_from_schema[1:]
...
def ingest_summary_files_multiple(
    con, db_path, table_name, file_list, schema, sample_map
):
    """
    ...
    for idx, file_path in enumerate(file_list):
        print(
            f"  Processing file {idx+1}/{len(file_list)}: {os.path.basename(file_path)}"
        )
...
def ingest_summary_file(
    con, db_path, table_name, file_path, schema, sample_map, skip_if_exists=True
):
    """
    ...
    if not file_path or not os.path.exists(file_path):
        print(
            f"  INFO: File for '{table_name}' not found in manifest or at path. Skipping."
        )
        return
...
def process_cache_files(con, cur, cache_path_map, sample_map):
    ...
    for i, (sample_name, cache_file_path) in enumerate(cache_path_map.items()):
        print(
            f"  Processing cache file {i + 1}/{file_count}: {os.path.basename(cache_file_path)} for sample {sample_name}..."
        )
        if not os.path.exists(cache_file_path):
            print(
                f"    WARNING: Cache file for sample '{sample_name}' not found at '{cache_file_path}'. Skipping."
            )
            continue
...
def _ingest_junction_data_for_sample(cur, sample_id_int, junction_metrics):
    ...
    if summary_data:
        cur.executemany(
            "INSERT INTO transcript_junction_summary VALUES (?, ?, ?, ?, ?, ?, ?)",
            summary_data,
        )
    ...
    if counts_data_to_insert:
        ...
        cur.executemany(
            "INSERT INTO junction_counts VALUES (?, ?, ?)", counts_data_to_insert
        )
...
def _ingest_coverage_data_for_sample(cur, sample_id_int, coverage_metrics):
    ...
    if coverage_data_to_insert:
        cur.executemany(
            "INSERT INTO territory_coverage VALUES (?, ?, ?)", coverage_data_to_insert
        )
...
def create_indexes(cur):
    ...
    index_table(
        "counts_with_confidence",
        ["sample_id_int", "transcript_id_int"],
        ["transcript_id_int"],
    )
    index_table(
        "final_counts_tpm",
    ...
    index_table(
        ...
        ["transcript_id_int"],
        ["gene_id", "transcript_id_str"],
    )
    index_table(
        "subset_supersets",
        ["subset_transcript_id_int", "superset_transcript_id_int"],
        ["superset_transcript_id_int"],
    )
    index_table("junction_annotations", ["junction_id_int"], ["junction_id_str"])

    # --- Indexing Junction Data Tables ---
    index_table(
        "transcript_junction_summary",
        ["sample_id_int", "transcript_id_int"],
        ["transcript_id_int"],
    )
    ...
    index_table(
        ...
        ["transcript_id_int", "junction_id_int"],
        ["junction_id_int"],
    )
    ...
    index_table(
        "transcript_territories", ["transcript_id_int", "region_id"], ["region_id"]
    )
...
def main():
    ...
    parser.add_argument(
        "--output_db", required=True, help="Path to the output SQLite database file."
    )
    parser.add_argument(
        "--metadata_file",
    ...
    if "per_sample_files" in manifest and manifest["per_sample_files"]:
        ...
        print(
            "ERROR: Cannot derive sample list. Manifest is missing 'per_sample_files' or 'cache_files' keys.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not annotation_path or not os.path.exists(annotation_path):
        print(
            f"ERROR: Annotation file specified in manifest not found at: {annotation_path}",
            file=sys.stderr,
        )
        print(
            "       If you have moved the project directory, please edit the paths in the manifest file.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        if args.metadata_file:
            if not os.path.exists(args.metadata_file):
                print(
                    f"FATAL ERROR: Specified metadata file not found at '{args.metadata_file}'",
                    file=sys.stderr,
                )
                sys.exit(1)
            sample_map = parse_and_ingest_metadata(
                args.output_db, args.metadata_file, sample_names
            )
            if not sample_map:
                print(
                    "FATAL ERROR: Metadata ingestion failed. Exiting.", file=sys.stderr
                )
                sys.exit(1)
        else:
            sample_map = extract_manifest_metadata(
                con, cur, args.metadata_file, sample_names
            )
        ...
        ingest_per_sample_data(
            con,
            cur,
            args.output_db,
            "counts_with_confidence",
            per_sample_files_map,
            "metrics_sparse",  # <-- file_key (key from manifest dictionary)
            COUNTS_WITH_CONFIDENCE_SCHEMA,
            sample_map,
            transcript_map,
        )
        ...
        cur.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_transcript_id_map_str ON transcript_id_map (transcript_id_str);"
        )
        cur.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_transcript_id_map_int ON transcript_id_map (transcript_id_int);"
        )
        cur.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_junction_id_map_str ON junction_id_map (junction_id_str);"
        )
        cur.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_junction_id_map_int ON junction_id_map (junction_id_int);"
        )
