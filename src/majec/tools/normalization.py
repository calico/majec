...
def calculate_median_of_ratios(db_path):
    ...
    try:
        ...
        counts_matrix = df_long.pivot(index='group_id', columns='sample_id', values='count').fillna(0)
        
        # Filter out genes that are zero across all samples
        counts_matrix = counts_matrix.loc[counts_matrix.sum(axis=1) > 0]
        print(f"  -> Matrix created with {counts_matrix.shape[0]} genes and {counts_matrix.shape[1]} samples.")
        ...
        df_samples = pd.read_sql_query("SELECT sample_id, sample_id_int FROM sample_metadata", con)
        df_factors = pd.merge(df_factors, df_samples, on='sample_id')
        
        # Use INSERT OR REPLACE to make the script safely re-runnable
        data_to_insert = [(row['sample_id_int'], row['size_factor']) for _, row in df_factors.iterrows()]
        cur.executemany("INSERT OR REPLACE INTO sample_normalization (sample_id_int, size_factor) VALUES (?, ?)", data_to_insert)
        ...
        con.close()
def main():
    parser = argparse.ArgumentParser(description="Calculate and add normalization size factors to a MAJEC database.")
    parser.add_argument("--db", required=True, help="Path to the MAJEC SQLite database file.")
