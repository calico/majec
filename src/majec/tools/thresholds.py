...
def run_featurecounts_for_sample(args_tuple):
    ...
    try:
        ...
        counts_df = pd.read_csv(
            output_path, sep="\t", comment="#", usecols=["Geneid", bam_path]
        )
...
def main():
    ...
    parser.add_argument(
        "--gene_gtf",
        required=True,
        help="Path to the primary gene annotation GTF file.",
    )
    parser.add_argument(
        "--te_gtf",
        default=None,
        help="Path to the TE annotation GTF file (optional). If provided, it will be combined with the gene GTF.",
    )
    # --- END CHANGE ---
    parser.add_argument(
        "--output",
        required=True,
        help="Path for the output thresholds JSON file (e.g., 'global_bins.json').",
    )
    parser.add_argument(
        "--threads",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        choices=[0, 1, 2],
        help="Strand specificity (default: 2).",
    )
    ...
    with tempfile.TemporaryDirectory(dir=args.tempdir) as temp_dir_main:
        ...
        if args.te_gtf:
            ...
            try:
                ...
                with open(gtf_to_use, "wb") as outfile:
                    subprocess.run(
                        ["cat", args.gene_gtf, args.te_gtf], stdout=outfile, check=True
                    )
                print(f"  Combined GTF created at: {gtf_to_use}")
            except (subprocess.CalledProcessError, FileNotFoundError) as e:
                print(
                    f"FATAL: Failed to combine GTF files. Error: {e}", file=sys.stderr
                )
                sys.exit(1)
        else:
        ...
        print(
            f"Processing {len(args.bams)} BAM files using {args.threads} parallel jobs."
        )
        ...
        if len(expressed_counts) == 0:
            print(
                "FATAL: No expressed features found across any samples. Cannot calculate thresholds.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        print(
            f"You can now use this file with the --use_bins flag in the main MAJEC pipeline."
        )
