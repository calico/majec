...
def get_cDNA_fragment_stats(
    ...
    try:
        ...
        if mean_str == "NA":
            logging.warning(
                f"No valid intra-exonic pairs found for {bam_path.name} within the range [{min_len}, {max_len}]. Cannot calculate stats."
            )
            return (
                output_path,
                False,
                f"No valid pairs found in range [{min_len}, {max_len}]",
            )
...
def process_bam_list(
    ...
    bam_paths = [
        Path(line.strip())
        for line in fileinput.input(files=input_source)
        if line.strip()
    ]
    ...
    logging.info(
        f"Found {len(valid_bams)} BAM files. Processing with {num_workers} parallel workers."
    )
